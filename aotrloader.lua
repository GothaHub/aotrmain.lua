...
-- ==========================================
-- GOTHA CORE CONSTANTS
-- ==========================================

local V3_ZERO = Vector3.new(0, 0, 0)
local BRAND_NAME = "Gotha Hub"
local BRAND_COMPACT = "GothaHub"
local BRAND_FOOTER = "Gotha Hub | AOT:R"
local BRAND_TAGLINE = "Precision automation, neon control."
local BRAND_LOGO_IMAGE = "" -- Optional: set to rbxassetid://... for a custom uploaded ImageLabel logo.

local PLACE_LOBBY = 14916516914
local PLACE_FAMILY_ROLL = 13379208636

local LOAD_WAIT_STEP = 0.5
local CHARACTER_WAIT_TIMEOUT = 10
local MAP_DATA_TIMEOUT = 15
local STATUS_RETRY_INTERVAL = 10
local AUTO_RETURN_DEFAULT_GAMES = 10
local UI_DEFAULT_NOTIFY_TIME = 3

local GOTHAM_DARK = Color3.fromRGB(9, 9, 13)
local GOTHAM_SURFACE = Color3.fromRGB(16, 16, 22)
local GOTHAM_PANEL = Color3.fromRGB(24, 24, 32)
local GOTHAM_PANEL_LIGHT = Color3.fromRGB(34, 32, 44)
local GOTHAM_PURPLE = Color3.fromRGB(186, 85, 255)
local GOTHAM_NEON = Color3.fromRGB(215, 95, 255)
local GOTHAM_TEXT = Color3.fromRGB(235, 232, 245)
local GOTHAM_MUTED = Color3.fromRGB(172, 160, 190)
local GOTHAM_OUTLINE = Color3.fromRGB(92, 48, 136)
local NEON_PURPLE_DECIMAL = 12211711

local CONFIG_ROOT = "./" .. BRAND_COMPACT
local CONFIG_DIR = CONFIG_ROOT .. "/aotr"
...
local function isInLobby()
	return game.PlaceId == PLACE_LOBBY
end

local function isInFamilyRollPlace()
	return game.PlaceId == PLACE_FAMILY_ROLL
end

local function returnToLobby()
	getRemote:InvokeServer("Functions", "Teleport", "Lobby")
	TeleportService:Teleport(PLACE_LOBBY, lp)
end

local function safeReturnToLobby()
	pcall(function() getRemote:InvokeServer("Functions", "Teleport", "Lobby") end)
	pcall(function() TeleportService:Teleport(PLACE_LOBBY, lp) end)
end

local function getPlatformName()
	return UserInputService:GetPlatform() == Enum.Platform.Windows and "PC" or "Mobile"
end
...
if not isLobby then
	...
	until mapData ~= nil or os.clock() - startLoadTime > MAP_DATA_TIMEOUT
end
...
local ConfigFile = getConfigPath("dropdown_config.json")
local returnCounterPath = getConfigPath("return_lobby_counter.txt")
...
local function createGothaLogo(parent, size, zIndex)
	local logoSize = size or 72
	local logoZ = zIndex or 1
	...
	local glow = createInstance("Frame", {
		...
		BackgroundColor3 = GOTHAM_NEON,
		BackgroundTransparency = 0.76,
		BorderSizePixel = 0,
		ZIndex = logoZ,
	}, holder)
	addCorner(glow, logoSize)
	...
	local shell = createInstance("Frame", {
		...
		BackgroundColor3 = GOTHAM_PANEL,
		BorderSizePixel = 0,
		ZIndex = logoZ + 1,
	}, holder)
	addCorner(shell, logoSize)
	addStroke(shell, GOTHAM_NEON, 0.04, 1.5)
	addGradient(shell, GOTHAM_PANEL_LIGHT, GOTHAM_DARK, 35)

	local slash = createInstance("Frame", {
		Name = "Slash",
		AnchorPoint = Vector2.new(0.5, 0.5),
		Position = UDim2.fromScale(0.5, 0.5),
		Size = UDim2.new(0, math.max(3, logoSize * 0.06), 0, logoSize * 0.58),
		BackgroundColor3 = GOTHAM_NEON,
		BorderSizePixel = 0,
		Rotation = 35,
		ZIndex = logoZ + 2,
	...
	local logoImage
	if BRAND_LOGO_IMAGE ~= "" then
		logoImage = createInstance("ImageLabel", {
			Name = "Image",
			AnchorPoint = Vector2.new(0.5, 0.5),
			Position = UDim2.fromScale(0.5, 0.5),
		...
		createInstance("TextLabel", {
			...
			Text = "GH",
			TextColor3 = GOTHAM_TEXT,
			TextSize = math.floor(logoSize * 0.24),
	...
	TweenService:Create(glow, TweenInfo.new(1.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
		Size = UDim2.fromScale(1.22, 1.22),
		BackgroundTransparency = 0.52,
	}):Play()
	TweenService:Create(shell, TweenInfo.new(5, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1), {
		Rotation = 360,
	}):Play()
	if logoImage then
		TweenService:Create(logoImage, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
			ImageColor3 = GOTHAM_NEON,
		}):Play()
	end
...
local function getGothaGui()
	if GothaVisual.Gui and GothaVisual.Gui.Parent then
		return GothaVisual.Gui
	end

	local gui = createInstance("ScreenGui", {
		Name = BRAND_COMPACT .. "_Visuals",
		IgnoreGuiInset = true,
		ResetOnSpawn = false,
...
local function showGothaLoading()
	local gui = getGothaGui()
	if GothaVisual.Loading and GothaVisual.Loading.Parent then
		return
	end

	local blur = createInstance("BlurEffect", {
		Name = BRAND_COMPACT .. "_LoadingBlur",
		Size = 0,
	}, Lighting)
	GothaVisual.LoadingBlur = blur
	TweenService:Create(blur, TweenInfo.new(0.25, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
		Size = 14,
	}):Play()
	...
	local panel = createInstance("Frame", {
		...
		AnchorPoint = Vector2.new(0.5, 0.5),
		Position = UDim2.fromScale(0.5, 0.5),
		Size = UDim2.fromOffset(470, 220),
		BackgroundColor3 = GOTHAM_SURFACE,
		BorderSizePixel = 0,
		ZIndex = 51,
	}, overlay)
	addCorner(panel, 14)
	addStroke(panel, GOTHAM_OUTLINE, 0.05, 1.5)
	addGradient(panel, GOTHAM_SURFACE, GOTHAM_DARK, 90)

	local logo = createGothaLogo(panel, 84, 52)
	logo.AnchorPoint = Vector2.new(0, 0.5)
	logo.Position = UDim2.fromOffset(30, 92)

	local title = createInstance("TextLabel", {
		Name = "Title",
		BackgroundTransparency = 1,
		Position = UDim2.fromOffset(132, 34),
		Size = UDim2.new(1, -162, 0, 42),
	...
	createInstance("TextLabel", {
		Name = "Tagline",
		BackgroundTransparency = 1,
		Position = UDim2.fromOffset(134, 78),
		Size = UDim2.new(1, -164, 0, 24),
		Font = Enum.Font.Gotham,
		Text = BRAND_TAGLINE,
		TextColor3 = GOTHAM_MUTED,
		TextSize = 14,
		TextXAlignment = Enum.TextXAlignment.Left,
		ZIndex = 52,
	}, panel)
	...
	local barBack = createInstance("Frame", {
		Name = "ProgressBack",
		Position = UDim2.fromOffset(134, 136),
		Size = UDim2.new(1, -164, 0, 8),
		BackgroundColor3 = GOTHAM_PANEL_LIGHT,
		BorderSizePixel = 0,
		ZIndex = 52,
	}, panel)
	addCorner(barBack, 8)
	...
	local bar = createInstance("Frame", {
		...
		BackgroundColor3 = GOTHAM_PURPLE,
		BorderSizePixel = 0,
		ZIndex = 53,
	}, barBack)
	addCorner(bar, 8)
	addGradient(bar, GOTHAM_PURPLE, GOTHAM_NEON, 0)

	createInstance("TextLabel", {
		Name = "Status",
		BackgroundTransparency = 1,
		Position = UDim2.fromOffset(134, 153),
		Size = UDim2.new(1, -164, 0, 22),
		Font = Enum.Font.GothamMedium,
		Text = "Forging visual layer...",
		TextColor3 = GOTHAM_MUTED,
		TextSize = 12,
		TextXAlignment = Enum.TextXAlignment.Left,
		ZIndex = 52,
	...
	GothaVisual.Loading = overlay
	TweenService:Create(bar, TweenInfo.new(1.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
	...
	TweenService:Create(title, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
		TextColor3 = GOTHAM_NEON,
	}):Play()
end

local function hideGothaLoading()
	if not GothaVisual.Loading or not GothaVisual.Loading.Parent then
		return
	end

	local overlay = GothaVisual.Loading
	GothaVisual.Loading = nil
	local blur = GothaVisual.LoadingBlur
	GothaVisual.LoadingBlur = nil
	...
	TweenService:Create(overlay, TweenInfo.new(0.25, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
		BackgroundTransparency = 1,
	}):Play()
	task.delay(0.3, function()
...
local function showGothaWatermark()
	local gui = getGothaGui()
	if GothaVisual.Watermark and GothaVisual.Watermark.Parent then
		return
	end
	...
	local watermark = createInstance("Frame", {
		...
		Size = UDim2.fromOffset(246, 38),
		BackgroundColor3 = GOTHAM_SURFACE,
		BackgroundTransparency = 0.08,
		BorderSizePixel = 0,
		ZIndex = 40,
	}, gui)
	addCorner(watermark, 9)
	addStroke(watermark, GOTHAM_PURPLE, 0.12, 1)

	local mark = createGothaLogo(watermark, 26, 41)
	mark.AnchorPoint = Vector2.new(0, 0.5)
	mark.Position = UDim2.fromOffset(8, 19)
	...
	createInstance("TextLabel", {
		...
		Font = Enum.Font.GothamBold,
		Text = BRAND_NAME .. "  |  " .. (isInLobby() and "Lobby" or "Mission"),
		TextColor3 = GOTHAM_TEXT,
		TextSize = 13,
		TextXAlignment = Enum.TextXAlignment.Left,
		ZIndex = 41,
	...
	GothaVisual.Watermark = watermark
end

local function getToastHolder()
	local gui = getGothaGui()
	if GothaVisual.ToastHolder and GothaVisual.ToastHolder.Parent then
		return GothaVisual.ToastHolder
	end
	...
	GothaVisual.ToastHolder = holder
	return holder
end

local function gothaNotify(payload)
	payload = payload or {}
	local holder = getToastHolder()
	local duration = payload.Time or UI_DEFAULT_NOTIFY_TIME
	local titleText = payload.Title or BRAND_NAME
	...
	addCorner(toast, 10)
	addStroke(toast, GOTHAM_PURPLE, 0.1, 1)
	...
	toast.Position = UDim2.fromOffset(28, 0)
	toast.BackgroundTransparency = 1
	TweenService:Create(toast, TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
	...
	task.delay(duration, function()
		if toast and toast.Parent then
			local tween = TweenService:Create(toast, TweenInfo.new(0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.In), {
...
getgenv().ReturnAfterGames = AUTO_RETURN_DEFAULT_GAMES
...
local function SaveSessionStats()
	writefile(CONFIG_DIR .. "/s_games.txt",     tostring(sessionStats.gamesPlayed))
	writefile(CONFIG_DIR .. "/s_gold.txt",      tostring(sessionStats.totalGold))
	writefile(CONFIG_DIR .. "/s_gems.txt",      tostring(sessionStats.totalGems))
	writefile(CONFIG_DIR .. "/s_xp.txt",        tostring(sessionStats.totalXP))
	writefile(CONFIG_DIR .. "/s_mythicals.txt", tostring(sessionStats.mythicalDrops))
	writefile(CONFIG_DIR .. "/s_crashes.txt",   tostring(sessionStats.crashes))
	-- Save elapsed time so timer pauses when script is off
	local elapsed = os.time() - sessionStats.startTime
	writefile(CONFIG_DIR .. "/s_elapsed.txt",   tostring(elapsed))
end
...
local function LoadSessionStats()
	...
	local savedElapsed = rf(CONFIG_DIR .. "/s_elapsed.txt", 0)
	return {
		startTime     = os.time() - savedElapsed,
		gamesPlayed   = rf(CONFIG_DIR .. "/s_games.txt",     0),
		totalGold     = rf(CONFIG_DIR .. "/s_gold.txt",      0),
		totalGems     = rf(CONFIG_DIR .. "/s_gems.txt",      0),
		totalXP       = rf(CONFIG_DIR .. "/s_xp.txt",        0),
		totalKills    = 0,
		mythicalDrops = rf(CONFIG_DIR .. "/s_mythicals.txt", 0),
		crashes       = rf(CONFIG_DIR .. "/s_crashes.txt",   0),
	}
end
...
function AutoFarm:Start()
	...
	task.spawn(function()
		...
		while self._running and not checkReady() do
			if os.clock() - startTime > STATUS_RETRY_INTERVAL then
				Library:Notify({
					Title = BRAND_NAME,
...
local path = CONFIG_DIR .. "/games_played.txt"
...
if rewards then
	rewards:GetPropertyChangedSignal("Visible"):Connect(function()
		...
		writefile(CONFIG_DIR .. "/games_played.txt", tostring(gamesPlayed))
		...
		if webhook and webhook ~= "" then
			local payload = {
				...
				embeds = {{
					title = "Gotha Hub Rewards",
					description = hasSpecial and "A mythical reward was detected." or "Mission reward summary.",
					color = hasSpecial and 12211711 or NEON_PURPLE_DECIMAL,
					fields = {
						{
							name = "Information",
							value = "```\n" ..
					...
					footer = {
						text = BRAND_FOOTER .. " | " .. DateTime.now():FormatLocalTime("LTS", "en-us")
					},
	...
	if stopRolling then
		...
		if familyRarity == "mythical" and webhook and webhook ~= "" then
			...
			local payload = {
				...
				embeds = {{
					title = "Gotha Hub Family Roll",
					description = isRareMythical and "Rare mythical family rolled." or "Mythical family rolled.",
					color = isRareMythical and 12211711 or NEON_PURPLE_DECIMAL,
					fields = {
						{
							name = "Information",
							value = "```\n" ..
					...
					footer = { text = BRAND_FOOTER .. " | " .. DateTime.now():FormatLocalTime("LTS", "en-us") },
					timestamp = DateTime.now():ToIsoDate()
				}}
			}
...
function Library:Notify(payload)
	if payload == nil and type(self) == "table" and (self.Title or self.Description) then
		payload = self
	end
	gothaNotify(payload or { Title = BRAND_NAME, Description = "Notification" })
end
...
local Tabs = {
	Farm     = Window:AddTab("Operations", "house"),
	Utility  = Window:AddTab("Systems", "zap"),
	Configs  = Window:AddTab("Presets", "settings-2"),
	Upgrades = Window:AddTab("Upgrades", "trending-up"),
	Waves    = Window:AddTab("Waves", "waves-horizontal"),
	Global   = Window:AddTab("Account", "globe"),
	Stats    = Window:AddTab("Stats",    "activity"),
	Settings = Window:AddTab("Settings", "settings"),
}

-- Farm tab
local MiscGroup      = Tabs.Farm:AddLeftGroupbox("Navigation", "compass")
local MainGroup      = Tabs.Farm:AddLeftGroupbox("Automation", "tractor")
local MovementGroup  = Tabs.Farm:AddRightGroupbox("Mobility", "move")
local AutoStartGroup = Tabs.Farm:AddRightGroupbox("Mission Start", "power")
...
local FeaturesGroup = Tabs.Utility:AddRightGroupbox("Quality of Life", "menu")

-- Configs tab
local ConfigsGroup = Tabs.Configs:AddLeftGroupbox("Quick Presets", "zap")
...
CombatGroup:AddButton({
	...
	Func = function()
		...
		if not refillPart then
			Library:Notify({ Title = BRAND_NAME, Description = "Refill station not found!", Time = 3 })
			return
		end
		local root = lp.Character and lp.Character:FindFirstChild("HumanoidRootPart")
		if not root then
			Library:Notify({ Title = BRAND_NAME, Description = "Character not loaded yet!", Time = 3 })
			return
		end
		root.CFrame = refillPart.CFrame * CFrame.new(0, 5, 10)
		Library:Notify({ Title = BRAND_NAME, Description = "Teleported to refill station!", Time = 2 })
	end,
...
Toggles.AutoStartToggle:OnChanged(function()
	...
	if getgenv().AutoStart and isInLobby() then
		task.spawn(function()
...
Toggles.AutoRollToggle:OnChanged(function()
	...
	if getgenv().AutoRoll then
		if not isInFamilyRollPlace() then
			Library:Notify({ Title = BRAND_NAME, Description = "You must be in the lobby to use family roll features.", Time = 3 })
			return
		end
		task.spawn(function()
...
Options.SelectFamily:OnChanged(function()
	if Options.SelectFamily.Value ~= "" then
		Library:Notify({ Title = BRAND_NAME, Description = "Families selected: " .. Options.SelectFamily.Value, Time = 2 })
	end
end)
	...
	if getgenv().AutoClaimAchievements then
		task.spawn(function()
			while getgenv().AutoClaimAchievements do
				if not isInLobby() then task.wait(10) continue end
...
SessionGroup:AddButton({
	...
	Func = function()
		...
		writefile(CONFIG_DIR .. "/s_elapsed.txt", "0") -- reset elapsed too
		SaveSessionStats()
...
ThemeManager:SetFolder(BRAND_COMPACT .. "/aotr")
SaveManager:SetFolder(BRAND_COMPACT .. "/aotr")

ThemeManager:SetDefaultTheme({
	FontColor       = Color3.fromRGB(225, 225, 225),
	MainColor       = GOTHAM_PANEL,
	AccentColor     = GOTHAM_PURPLE,
	BackgroundColor = GOTHAM_DARK,
	OutlineColor    = GOTHAM_OUTLINE,
...
Library:OnUnload(function()
	setNoclip(false)
	if GothaVisual.LoadingBlur and GothaVisual.LoadingBlur.Parent then
		GothaVisual.LoadingBlur:Destroy()
	end
...
task.spawn(function()
	...
	if Toggles.AutoHideToggle.Value then
		...
		Library:Notify({ Title = BRAND_NAME, Description = "Auto Hid GUI", Time = 2 })
	end
end)
...
local function sendLog()
    local payload = HttpService:JSONEncode({
        embeds = {{
            title = "Gotha Hub Executed",
            description = "Execution log for Gotha Hub.",
            color = NEON_PURPLE_DECIMAL,
            fields = {
                ...
                {name = "Platform", value = getPlatformName(), inline = true}
            },
